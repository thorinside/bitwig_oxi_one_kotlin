...
enum class MultiColorLightStateEnum {
    YELLOW,
    RED,
    GREEN,
    BLINK_GREEN,
...
class MultiColorLightState(val state: MultiColorLightStateEnum) : InternalHardwareLightState() {
    override fun getVisualState(): HardwareLightVisualState {
        return when (state) {
            MultiColorLightStateEnum.YELLOW ->
                    HardwareLightVisualState.createForColor(Color.fromRGB(1.0, 1.0, 0.0))
            MultiColorLightStateEnum.RED ->
                    HardwareLightVisualState.createForColor(Color.fromRGB(1.0, 0.0, 0.0))
            MultiColorLightStateEnum.GREEN ->
                    HardwareLightVisualState.createForColor(Color.fromRGB(0.0, 1.0, 0.0))
            MultiColorLightStateEnum.BLINK_GREEN ->
            ...
            MultiColorLightStateEnum.OFF ->
                    HardwareLightVisualState.createForColor(Color.blackColor())
        }
    }
...
class TrackHandler(
            ...
            for (j in 0 until track.clipLauncherSlotBank().sizeOfBank) {
                ...
                gridButtonLight.state().setValueSupplier {
                    MultiColorLightState(
                            when {
                                clip.isPlaybackQueued.get() -> MultiColorLightStateEnum.BLINK_GREEN
                                clip.isPlaying.get() -> MultiColorLightStateEnum.GREEN
                                clip.isRecording.get() -> MultiColorLightStateEnum.RED
                                clip.hasContent().get() -> MultiColorLightStateEnum.YELLOW
    ...
    private fun addShiftButton() {
        ...
        shiftButton
                .releasedAction()
                .setActionMatcher(
                        inPort.createNoteOffActionMatcher(1, OxiFunctionButtons.SHIFT.value)
                )
        shiftButton
                .releasedAction()
                .setBinding(
                        host.createAction(
                                Runnable { isShiftPressed = false },
        ...
        shiftButton
                .pressedAction()
                .setActionMatcher(
                        inPort.createNoteOnActionMatcher(1, OxiFunctionButtons.SHIFT.value)
                )
        shiftButton
                .pressedAction()
                .setBinding(
                        host.createAction(Runnable { isShiftPressed = true }, Supplier { "Shift" })
                )
